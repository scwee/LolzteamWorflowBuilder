...
Пошаговый гайд: поднять builder на сервере. Логина нет — после установки сразу UI → токен → сценарии.
...
- Ограничьте доступ к серверу (firewall / VPN / закрытый хост). Не публикуйте IP без необходимости.
