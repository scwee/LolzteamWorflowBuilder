...
| Запустить **локально** для разработки | [Шаг B](#b-локальный-запуск-разработка) |
...
`SECRETS_ENCRYPTION_KEY` можно оставить пустым **только в локальной разработке** (не на публичном сервере).
